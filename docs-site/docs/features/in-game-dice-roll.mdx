...
1. Choose `2 个骰子 · 掷两次` or `3 个骰子 · 掷一次`; two dice is selected by default.
2. Tap the center tile to roll.
3. In two-dice mode, the first total selects the seat and the selected player rolls a second time.
4. In three-dice mode, 東 rolls once and that total selects both the seat and break count.
5. Read the highlighted opening seat, break position, and the next stack for tile pick-up.
...
4. A fresh completed result resets the next time the center tile is used.
5. The wall color theme rotates between rolls.
